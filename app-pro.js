...
function drawDailyPoint() {
  ...
  const byStudent = studentIds.map((id) => {
    ...
    return {
      ...
      receipts: dayPayments.map((item) => item.receiptNo || item.id).join(", ")
    };
  });
